...
How it works:
  ...
  status   — summarise pending vs graded, and model-vs-floor on graded entries.
...
def status() -> None:
    ...
    if graded:
        ...
        def mean(k):
            return st.mean(e["graded"][k] for e in graded)
...
def main() -> None:
    ...
    {"predict": predict, "grade": grade, "verify": verify, "status": status}.get(
        cmd, lambda: print(f"unknown command {cmd!r}; use predict|grade|verify|status"))()
